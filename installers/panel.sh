...
pcdl_dl() {
  output "Downloading pelican github latest panel files .. "
...
install_peliq() {
  output "Installing peliq service.."

  curl -o /etc/systemd/system/peliq.service "$GITHUB_URL"/configs/peliq.service

  case "$OS" in
  debian | ubuntu)
    sed -i -e "s@<user>@www-data@g" /etc/systemd/system/peliq.service
    ;;
  rocky | almalinux)
    sed -i -e "s@<user>@nginx@g" /etc/systemd/system/peliq.service
    ;;
  esac

  systemctl enable peliq.service
  systemctl start peliq

  success "Installed peliq!"
}
